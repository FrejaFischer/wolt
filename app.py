from bottle import default_app, get, post, request, response, run, template, static_file, redirect, delete
...
def _(user_pk):
    try:
        ...
        db = x.db()
        q = db.execute("DELETE FROM users WHERE user_pk = :user_pk", user)
        if q.rowcount == 0: #can be used in any of these UPDATE DELETE INSERT
            raise Exception("User could not be deleted", 404)
        db.commit()
        toast = template("__toast.html", message="User deleted")
        return f"""
...
def _(user_pk):
    try:
        ...
        db = x.db()
        q = db.execute("UPDATE users SET user_blocked_at = :user_blocked_at WHERE user_pk = :user_pk AND user_blocked_at = 0", user)
        if q.rowcount == 0: #can be used in any of these UPDATE DELETE INSERT
...
def _(user_pk):
    try:
        ...
        db = x.db()
        q = db.execute("UPDATE users SET user_blocked_at = 0 WHERE user_pk = :user_pk AND user_blocked_at != 0", user)
        if q.rowcount == 0:
            raise Exception("User could not be blocked", 404)
        db.commit()
